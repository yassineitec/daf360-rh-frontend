 ...
 *
 * Stateless with respect to the data: columns, drag state and the loading flag
 * all come in as inputs and every interaction goes back out as an output. The
 * one thing it does own is its **own scroll geometry** — the board element lives
 * here, so the horizontal navigation minimap that reads `scrollLeft` /
 * `scrollWidth` belongs here too rather than in the page.
 */
@Component({
