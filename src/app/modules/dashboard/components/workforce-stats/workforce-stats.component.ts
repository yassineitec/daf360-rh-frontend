...
@Component({
  ...
  template: `
    <div class="bg-surface-container-lowest p-6 rounded-xl border border-outline-variant
      ...
      <div>
        <p class="text-[11px] text-outline uppercase tracking-wider mb-2">Total Workforce</p>
        <p class="text-[20px] font-bold text-on-surface leading-snug">
          {{ totalActifs() }} Employés Actifs
        </p>
        <div class="mt-6 flex gap-8">
          <div class="flex flex-col">
            <p class="text-[11px] text-outline font-bold uppercase">Féminin</p>
            <p class="text-[18px] font-bold text-teal">
              {{ pctFemmes() != null ? pctFemmes() + '%' : '—' }}
            </p>
          </div>
          <div class="flex flex-col">
            <p class="text-[11px] text-outline font-bold uppercase">Masculin</p>
            <p class="text-[18px] font-bold text-teal">
